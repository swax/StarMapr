...
**Top Level**: `01_run_headshot_detection.py` - Complete end-to-end workflow
**Mid Level**: `02_run_actor_training.py` - Automated actor training + testing
**Low Level**: `05_run_pipeline_steps.py` - Manual step-by-step execution
...
└── 01_run_headshot_detection.py       # ★ PRIMARY ENTRY POINT
    ├── 02_run_actor_training.py   # ★ MID-LEVEL automation
...
1. **01_run_headshot_detection.py** - **PRIMARY ENTRY POINT**
   - Takes video URL and actors as input
   - Calls `02_run_actor_training.py` for each actor
...
2. **02_run_actor_training.py** - Automated actor training + testing
...
6. **10_download_actor_images.py** - Downloads test images (same as step 1)
7. **11_remove_dupe_training_images.py** - Removes test duplicates (same as step 2)
...
11. **30_download_video.py** - Downloads videos using yt-dlp
    - Supports YouTube, Vimeo, TikTok, saves to `05_videos/[site]_[id]/`

12. **31_extract_video_frames.py** - Extracts frames using binary search
    - Saves to `frames/` subfolder with zero-padded naming

13. **32_extract_frame_faces.py** - Detects faces in frames
...
18. **92_print_pkl.py** - Pickle file inspector
19. **00_run_integration_test.py** - Integration test script with mock data
